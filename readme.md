# SpongeBob J2ME Games
Mostly for old 2000s phones back in the day that only ran Java applets.
## Games
**Highly recommended just to download the whole repo; it's very small in size (~4 MB), making it very lightweight and portable.**
| Game                              | Screen Size       | Language               |
|-----------------------------------|-------------------|------------------------|
| Bikini Bottom Pursuit.jar         | 360x640           | en, es, pt             |
| Creature from the Krusty Krab.jar | 240x320           | en, fr, it, de, es     |
| DoodleJump.jar                    | 240x320           | en                     |
| Baytona 500.jar                   | 176x208           | en                     |
| Bowling.jar                       | 176x208           | en                     |
| Darts.jar (jad)                   | 128x128           | en                     |
| Movie.jar                         | 176x208           | en, de, fr, it, es     |
| Paparazzi Parade-128x128.jar      | 128x128, 128x160  | en, fr, it, de, es, ru |
| Paparazzi Parade-240x320.jar      | 240x320           | en, fr, it, de, es, ru |
| Racing (CN).jar                   | 176x208           | cn                     |
| Jump.jar                          | 240x320           | en, de, fr it, es      | 
| Legend of the Lost Spatula.jar    | 240x320           | en                     | 


## Archives
https://archive.org/details/HugeJavaMobileGameDump

https://www.mobiles24.co/downloads/tag/SpongeBob+SquarePants/free-java-games

https://mobile.phoneky.com/games/?q=sponge
